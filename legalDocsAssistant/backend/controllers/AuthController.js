...
export const updateStateRoute = async (req, res) => {
  const { email, state, typeOfDoc } = req.body;

  try {
    const user = await User.findOneAndUpdate({ email }, { state, typeOfDoc }, { new: true });

    if (!user) {
      return res.status(404).json({ message: "User not found" });
    }

    res.status(200).json({ message: "State and typeOfDoc updated successfully", user });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Failed to update state and typeOfDoc" });
  }
};
